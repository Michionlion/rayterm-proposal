...
When a photographic negative is taken, dark spots missed by the rays are now ``in shadow'', as the color levels were inverted. Today, Appel's work is not considered the first real ray-tracing algorithm, as it is almost in a different class. However, his work informed much of the following research, especially his ideas and mathematics on light intensity.
...
If it were possible to drastically reduce the number of tests completed for each ray, perhaps by only testing the ray against objects in its general vicinity, massive speed gains would emerge.
As it turns out, this was first thought of right after Whitted's original paper.
...
A BVH essentially groups objects into hierarchial organizations, with each group covering a larger area than the groups inside it.
Each group has a ``bounding volume'', a geometric primitive that encloses all members of that group.
Ray-tracing intersections are done with the root group first, progressing down the hierarchy and potentially skipping the vast majority of geometry in the scene, thereby speeding up computation.
...
Ray-tracing is inherently parallelizable, because each ray-tree is independent -- rays cannot collide, and one fragemenet's ray-tree does not affect another fragment's ray-tree.
Thus, there has been many opimizations that target enhanced hardware acceleration to enable massively parrallel ray-tracing.
The first example of this was {\it Design and Analysis of a Parallel Ray Tracing Computer}, a 1983 paper by John Cleary and others.
...
They were even able to build small prototypes, but calculated that a full blown system would cost \$50 million and be able to generate 1000 by 1000 images in 0.15 seconds.
Sadly, 1983 chip technology had not yet progressed to the point where such a large multi-core processor was feasable.
...
Called TerminalImageViewer (\texttt{tiv}) \cite{tivGithub}, it uses RGB ANSI codes and Unicode block graphic characters to display images in a terminal window.
...
This does not even mention the many other differences that exist between \texttt{tiv} and \name.
...
A stated goal of \name\ is to support future games that use the terminal as a graphical display.
...
It has a similar purpose to \name\ -- namely, facilitate game creation in the terminal.
However, Termloop, like all other terminal game engines available today, has one glaring limitation: it is not 3D.
This one differentiating factor is huge; it means that no games can ever be made that don't fit into the strict field of two-dimensional games.
\name\ changes that, by allowing full 3D rendering in the terminal.
It doesn't, of course, have the other niceties such as collision detection, but the ultimate goal of \name\ -- beyond the stated goals in this proposal -- is to grow to the same level of support for 3D games that Termloop has for 2D games.
...
Materials are not fully user-specifiable.
Instead, they simply reference a material type, such as lambertian or dielectric, and a texture.
...
Some additional values such as roughness control specific aspects of the generated scattering function.
...
Objects are composed of geometric primitives or other objects.
Since \name\ supports only spheres, infinite planes, and triangles, more complex objects require composition.
A ``box'' object for example might contain four ``side'' objects which themselves contain two triangles.
Each sub-object is associated with an offset from its parent.
Thus, the data required for an object definition is just an array of sub-objects and a position.
Every object may also have a material ID -- if not specified, the object's parent's material ID is used instead.
This material ID dictates what material is used to scatter rays when they intersect with the object.
Objects may also have an animation component -- a {\it movement path} -- ID.
Similar to materials, movement paths are simply a collection of locations and a length of time to take to traverse those locations.
In future versions of \name, movement paths are likely to be removed in favor of total object control in the form of scripting integration.
